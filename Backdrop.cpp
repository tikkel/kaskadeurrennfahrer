						...
						long viewpoint_z_angle)
{
	...
	on_screen = ClipLine(&x1, &y1, &x2, &y2);

	// printf("upside_down = %ld, on_screen = %ld\n", upside_down, on_screen);
	// printf("clipped (x1,y1) = (%ld,%ld), (x2,y2) = (%ld,%ld)\n\n", x1, y1, x2, y2);
	...
	if (on_screen)
	{
		...
		if (draw) // if previous rectangle was drawn
		{
			if (ybottom < max_y)
			{
				DrawFilledRectangle(min_x, ybottom, max_x, max_y, SCRGB(colour_index));
			}
		}
		else
		{
...
#define SCENERY_X_Y_SCALE_FACTOR (65536 / (4 * FOCUS)) // (z of 0x00010000, divided by (4 * FOCUS))
