...
static void GetSurfaceCoords(long piece, long segment)
{
	if ((segment < 0) || (segment >= Track[piece].numSegments))
		MessageBox(NULL, "GetSurfaceCoords(opponent) segment out of range", "Error", MB_OK);
...
static long Opponent_Speed_Value(long TrackID, long pos)
{
	...
	*/
	...
	long B63ce1 = (long)rand() & (long)opp_track_speed_values[TrackID + 16];
	B63ce1 += (long)opp_track_speed_values[TrackID + 24];
	long /*value,*/ d0;
	if (b < 0)
	{
		// value = B63ce1-10;
...
opi12:
	...
	if (d0 < 0)											 // temp, remove
		MessageBox(NULL, "Less than 0", "Error", MB_OK); // temp
	if (d0 >= 225)
		goto opi13;
