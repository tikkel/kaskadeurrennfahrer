...
#if defined(PANDORA) && defined(PYRA) && defined(ODROID) && defined(CHIP)
#define STBI_NEON
#endif
...
#else
#define MB_OK 1
#define MessageBox(a, text, type, button) printf("%s: %s\n", text, type)
#endif
...
public:
  ...
  void DrawText(const wchar_t *line);
  void DrawTextLine(const wchar_t *line);
  void DrawFormattedText(const wchar_t *line, ...);
...
private:
  ...
  int m_size;
  int m_fontsize;
  int m_posx, m_posy, s_posx, s_posy;
  float m_inv;
  int m_as[256];
  float m_forecol[4];
  GLuint m_texture;
