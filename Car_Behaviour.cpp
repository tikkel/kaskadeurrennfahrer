...
void ResetPlayer(bool newGame)
{
	cout << "reset player, damage reset: " << newGame << endl;
...
static void CalculateWorldRoadHeight(long wheel, long x, long z, long *y_out)
{
	...
	else
	{
		...
		if (calculated_segment >= Track[piece].numSegments)
			MessageBox(NULL, "calculated_segment out of range", "Error", MB_OK);
...
static void GetSurfaceCoords(long piece, long segment)
{
	if ((segment < 0) || (segment >= Track[piece].numSegments))
		MessageBox(NULL, "GetSurfaceCoords segment out of range", "Error", MB_OK);
...
long damaged_limit = 10; // Actually track dependant (could add to track data)

// NOTE: road_cushion_value is 0 for standard
...
void CompareAmigaWord(char *name, long amiga_value, long *value)
{
	...
	//	if (abs(*value - amiga_value) > 1)
	{
		...
		fprintf(out, "%s different %ld %ld (VALUE2 %d)\n", name, amiga_value, *value, VALUE2);
