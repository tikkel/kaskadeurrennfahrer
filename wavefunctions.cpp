...
IDirectSoundBuffer8 *MakeSoundBuffer(IDirectSound8 *ds, LPCWSTR lpSampleName)
{
	...
	if ((pRIFFBytes = GetWAVRes(lpSampleName)) != NULL)
	{
...
void *GetWAVRes(LPCWSTR lpResName)
{
	void *pRIFFBytes;
	...
	int i = 0;
	while (resname[i] && SDL_wcscasecmp(resname[i], lpResName))
		i++;
	if (!resname[i])
		return NULL;
	...
	if (fread(pRIFFBytes, 1, fsize, f) < fsize)
		printf("Couldn't read complete Wavefile: %s\n", filename[i]);
	else
		printf("Read Wavefile: %s\n", filename[i]);
	fclose(f);
